...
class AVPlayerView: UIView {
    ...
    weak var delegate:AVPlayerUpdateDelegate?    //代理
    ...
    init() {
        super.init(frame: CGRect(origin: .zero, size: CGSize(width: UIScreenWidth, height: UIScreenHeight)))
        initSubView()
    }
    ...
    func resourceLoader(_ resourceLoader: AVAssetResourceLoader, shouldWaitForLoadingOfRequestedResource loadingRequest: AVAssetResourceLoadingRequest) -> Bool {
        if task == nil {
            if let url = loadingRequest.request.url?.absoluteString.urlScheme(scheme: sourceScheme ?? "http") {
                let request = URLRequest.init(url: url, cachePolicy: URLRequest.CachePolicy.reloadIgnoringLocalCacheData, timeoutInterval: 15)
